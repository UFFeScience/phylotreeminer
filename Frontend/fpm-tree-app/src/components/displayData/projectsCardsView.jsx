...
const ProjectsCardsView = ({ projects, statusMap, onProjectSelect }) => {
    ...
    return (
        <Row gutter={[12, 12]}>
                ...
                <Col xs={8} sm={6} lg={6} key={job.name}>
                    ...
                    >
                        <Space direction="vertical" style={{ width: '100%' }}>
                            <Row justify="space-between"><Text type="secondary">Started</Text><Text>{new Date(job.last_modified).toLocaleString('pt-BR')}</Text></Row>
                            <Row justify="space-between"><Text type="secondary">Duration</Text><Text>{convert_time(job.duration) || '--'}</Text></Row>
                            ...
                            {
                                job.status === 'running' ? <Progress percent={progress_percent[job.details?.current_step]} status />
                                    : job.status === 'completed' ? <Progress percent={100} status />
                                        : job.status === 'failed' ? <Progress percent={100} status="exception" />
                                            : <Text type="secondary">Searching <LoadingOutlined /></Text>
                            }
                        </Space>
            ...
            )) : <Col span={24} style={{ textAlign: 'center', padding: 50 }}><Empty description="No projects found with applied filters." /></Col>}
        </Row>
    );
};
